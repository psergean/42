...
/*   Updated: 2017/10/05 00:00:44 by psergean         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fract				*init_julia_bis_fract(t_env *env)
{
	...
	fract->z_i = 0;
	fract->i = 0;
	fract->x1 = -1.8 + env->dec_x;
	fract->x2 = 1.8 + env->dec_x;
	fract->y1 = -1.7 + env->dec_y;
	fract->y2 = 1.7 + env->dec_y;
...
void				julia_bis(t_env *env)
{
	...
	x = 0;
	env->fract = init_julia_bis_fract(env);
	while (x < env->width)
	{
		y = 0;
		while (y < env->heigth)
